...
const wsLink = new WebSocketLink({
  ...
  uri: isDev
    ...
    : "ws://nuber-server.herokuapp.com/subscription",
});
...
const errorLink = onError(({ graphQLErrors, networkError }) => {
  if (graphQLErrors) {
    graphQLErrors.map(({ message }) => {
      return toast.error(`Unexpected error: ${message}`);
    });
  }
  if (networkError) {
...
const localStateLink = withClientState({
  ...
  resolvers: {
    Mutation: {
      logUserIn: (_: any, { token }: any, { cache: localCache }: any) => {
      ...
      logUserOut: (_: any, __: any, { cache: localCache }: any) => {
